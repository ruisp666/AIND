...
# We have used debugging through pirnt commands, rather than logging
class PgNode():
...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        while not leveled:
            ...
            level += 1
            #print("calling add_literal_level")
    ...
    def add_action_level(self, level):
        ...
        #append an empty set to represent the still empty new level
        self.a_levels.append(set()) 
         
        # go over all actions
        for a in self.all_actions:  
            #noop_actions(self, literal_list)
            #print(a)
            #if PgNode_a(a).action == "Bake(Cake,)":
               # print("Arrived")
               
            
            a_candidate= self.action_N_if_possible(level,a)
            #check if thep renodes of the action are contained in the previous
            #level, and return a PgNode_a object corresponding to a if True.
            if a_candidate:
                # for each prenode we add it to the parents
                for node in a_candidate.prenodes:
                    #a_candidate.parents.add(node)
                    #print("The candidate for A{} is....{}".format(level, a_candidate.action))
                    
                    a_candidate.parents.add(node)
                    # after adding the parents, check which from same s_level
                    #are prenodes of a_candidate
                    for node2 in self.s_levels[level]:
                        if node2 in a_candidate.prenodes:
                            node2.children.add(a_candidate)
                    #add the node with the parents and the children
                    self.a_levels[level].add(a_candidate) 
                    
                    
#        print("showing acumulated nodes at level {}".format(level))
#        for i in range(level+1):
#            print("++++at Level A{} from {}...".format(i,level))
            #for n in self.a_levels[i]:
                #n.show()
#        for i in range(level+1):
#            print("++++at Level S{} from {}...".format(i,level))
            #for n in self.s_levels[i]:
                #n.show()        
                
                
        #print("..............Returning to calculations..........")           
        
            
    def action_N_if_possible(self, level : int, action: Action):
        """Checks if the action is possible  in the current level, and 
        returns a PgNode_a if it is possible"""
        # check for positive clauses
       
        #create an object with the action   
        action_n=PgNode_a(action)
        #print("Arrived")
        #print(action_n.action)
#        if action_n.is_persistent:
#            print("Is persistent")
#            for el in action_n.prenodes:
#                #if el in self.s_levels[level]:
#                    el.show()
        #for node in :
                #node.show()
        
        #check if  prenodes of action_n is a subset of the same S level
        if not action_n.prenodes.issubset(self.s_levels[level]):
            return False
        return action_n
    ...
    def add_literal_level(self, level):
        ...
        #print("...........................Adding literals .............................on level S{}.".format(level))# S1 set of s_nodes - empty to start
        self.s_levels.append(set())

        for a in self.a_levels[level-1]:
#           print("")
#           #print("{}".format(a.action.name))
#           print("")
            
           # go over each node of a.effnodes
           for node2 in a.effnodes:
               #print(a.action)
               #print(node2.is_pos)
               #print(len(self.s_levels[level]), level)
#               if PgNode_s(node2.symbol,node2.is_pos) in self.s_levels[level]:
#                   print("Already there")
#               else:
#                   node2.show()
               
               #add node2 as children of node action a
               a.children.add(node2)
               
               #add the action a as a parent to node2
               node2.parents.add(a)
               self.s_levels[level].add(node2)
#               for b in self.a_levels[level-1]:
#                   if node2 in b.effnodes:
#                       node2.parents.add(b)
       # print("showing acumulated nodes at level {}".format(level))
#        for i in range(level+1):
#            print("++++at Level S{} from {}...".format(i,level))
...
#        print("..............Returning to calculations..........")
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #check if some literals from node_a1.effect_add is in node_a2.effect_rem
        for literal in node_a1.action.effect_add:
            if literal in node_a2.action.effect_rem:
                return True
                
        #check ig some literal from node_a2.effect_add is in in node_a1.effect_rem
        for literal in node_a2.action.effect_add:
            if literal in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # check all possible types of mutual exclusion between preconditions
        #and effects.
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """     
        #use parents to represent the preconditions
        for node in node_a1.parents:
           for node1 in node_a2.parents:
               if node1.is_mutex(node):
                   return True        
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # create new object as a compy of node_s2
        node_s3=PgNode_s(node_s2.symbol,node_s2.is_pos)
        
        #change the value of is_pos from node_s_3
        node_s3.is_pos= not node_s3.is_pos
        
        #compare  node_s_1 with node_s_3
        if node_s1.__eq__(node_s3):
            return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        #Use parents to check for possible actions with efeccts node_s1 
        #or node_s2
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        #The global counter of levels
        level_sum=0
        for literal in self.problem.goal:
            #check if the literal has been found
            found=False
            level=0
            # note that the level will not go above the levelling off level, so 
            # the next cycle terminates.
            while not found:
                #check if the literal as been found in the current S level
                #and if True, pass to the next
                #literal, and restart the search on level 0.
                if PgNode_s(literal,True) in self.s_levels[level]:
                    found=True
                    level_sum+=level
                #if not, increase the level and look again 
                else:
                    level+=1
        return level_sum
    
        ''' Before submitting, I came up with an alternative and probably faster way to calculate this
        heuristic. Namely, instead of looking through literals, check on each level if
        some goal is found, and if n_l goals are found on level $l$ add n_l*l
        to the counter. I tried the following, but it didn't seem to be working yet'''
