...
class PlanningGraph():
    ...
    def action_N_if_possible(self, level : int, action: Action):
        ...
        for el_node in action_n.prenodes:
            if el_node not in self.s_levels[level]:
               return False
        return action_n 
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set())  # S1 set of s_nodes - empty to start
        for a in self.a_levels[level-1]:
            for nod in a.effnodes:
                nod.parents.add(a)
                a.children.add(nod)
                self.s_levels[level].add(nod)
